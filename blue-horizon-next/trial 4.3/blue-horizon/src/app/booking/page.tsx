...
import { Calendar as CalendarIcon, Check, ArrowRight, ArrowLeft, MapPin, Star, BookOpen } from "lucide-react";
...
const COURSE_OPTIONS = [
    ...
    { value: "peak-performance-buoyancy", label: "Peak Performance Buoyancy" },
    { value: "divemaster", label: "Divemaster" },
    { value: "open-water-instructor", label: "Open Water Scuba Instructor" },
];

const SITE_OPTIONS = [
    { value: "blue-corner", label: "Blue Corner, Palau" },
    { value: "yongala", label: "SS Yongala, Australia" },
    { value: "blue-hole", label: "Great Blue Hole, Belize" },
    { value: "manta-point", label: "Manta Point, Indonesia" },
];
...
function BookingInner() {
    ...
    const [bookingType, setBookingType] = useState<"dive" | "course">(courseParam ? "course" : "dive");
    const [selectedCourse, setSelectedCourse] = useState(courseParam ?? "open-water-diver");
    ...
    const selectedCourseLabel = COURSE_OPTIONS.find((c) => c.value === selectedCourse)?.label ?? "Course";
    ...
    return (
        <div className="min-h-screen pb-20">
            ...
            <div className="relative container mx-auto px-6 max-w-6xl pt-32">
                ...
                <p className="text-slate-400 mb-10 text-lg" style={{ fontFamily: "system-ui, sans-serif" }}>
                    Secure your spot for an unforgettable underwater adventure.
                </p>

                {/* Booking type toggle */}
                <div className="flex gap-2 mb-12">
                    {(["dive", "course"] as const).map((type) => (
                        <button
                            key={type}
                        ...
                        >
                            {type === "dive" ? <MapPin className="w-4 h-4" /> : <BookOpen className="w-4 h-4" />}
                            {type === "dive" ? "Fun Dive" : "Course"}
                        </button>
                    ))}
                </div>

                {/* Course pre-selection banner */}
                {bookingType === "course" && (
                    <div className="mb-8 p-4 rounded-2xl border border-cyan-500/20 bg-cyan-950/20 flex items-center justify-between gap-4 flex-wrap">
                        <div className="flex items-center gap-3">
                            <BookOpen className="w-5 h-5 text-cyan-400 shrink-0" />
                            <div>
                                <p className="text-white font-semibold text-sm">{selectedCourseLabel}</p>
                                <p className="text-slate-400 text-xs" style={{ fontFamily: "system-ui, sans-serif" }}>
                                    In-water training booking — eLearning purchased separately via PADI
                                </p>
                            </div>
                        </div>
                        <Select value={selectedCourse} onValueChange={setSelectedCourse}>
                            <SelectTrigger className="w-56 bg-slate-800 border-slate-600 text-white text-sm h-9">
                                <SelectValue />
                            </SelectTrigger>
                        ...
                        {currentStep === 0 && (
                            <div className="space-y-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    {/* Site or course — only show site picker for fun dives */}
                                    {bookingType === "dive" ? (
                                        ...
                                        <div className="bg-slate-900/60 p-6 rounded-xl border border-cyan-500/20">
                                            <div className="flex items-center gap-3 mb-4">
                                                ...
                                                <label className="text-base font-semibold text-slate-200">Course</label>
                                            </div>
                                            <Select value={selectedCourse} onValueChange={setSelectedCourse}>
                                                <SelectTrigger className="w-full bg-slate-800 border-slate-600 text-white h-12">
                                                    <SelectValue />
                                    ...
                                    <div className="bg-slate-900/60 p-6 rounded-xl border border-blue-400/20 hover:border-blue-400/40 transition-colors">
                                        <div className="flex items-center gap-3 mb-4">
                                            ...
                                            <label className="text-base font-semibold text-slate-200">Start Date</label>
                                        </div>
                                        <Popover>
                                ...
                                <div className="bg-slate-900/60 p-6 rounded-xl border border-teal-400/20 hover:border-teal-400/40 transition-colors">
                                    <div className="flex items-center gap-3 mb-4">
                                        ...
                                        <div>
                                            <label className="block text-base font-semibold text-slate-200">Current Certification</label>
                                            <span className="text-xs text-slate-500" style={{ fontFamily: "system-ui, sans-serif" }}>
                                                Helps us match you with the right instructor.
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    {availableGear.map((item) => (
                                        ...
                                        >
                                            <div className="flex items-center gap-3">
                                                ...
                                                />
                                                <span className="text-slate-200 font-medium text-sm" style={{ fontFamily: "system-ui, sans-serif" }}>{item.name}</span>
                                            </div>
...
export default function Booking() {
    return (
        <Suspense fallback={<div className="min-h-screen" style={{ background: "#020d18" }} />}>
            <BookingInner />
        </Suspense>
    );
}
